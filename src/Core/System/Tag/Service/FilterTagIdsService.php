...
class FilterTagIdsService
{
    ...
    private function addEmptyFilter(QueryBuilder $query): void
    {
        ...
        $manyToManyFields = $this->tagDefinition->getFields()->filter(fn (Field $field): bool => $field instanceof ManyToManyAssociationField);
    ...
    {
        /** @var CompiledFieldCollection<ManyToManyAssociationField> $manyToManyFields */
        $manyToManyFields = $this->tagDefinition->getFields()->filter(fn (Field $field): bool => $field instanceof ManyToManyAssociationField && \in_array($field->getPropertyName(), $assignments, true));

...
class AppCheckoutGateway implements CheckoutGatewayInterface
{
    ...
    public function process(CheckoutGatewayPayloadStruct $payload): CheckoutGatewayResponse
    {
        ...
        $paymentMethods = $payload->getPaymentMethods()->map(fn (PaymentMethodEntity $paymentMethod): string => $paymentMethod->getTechnicalName());
        $shippingMethods = $payload->getShippingMethods()->map(fn (ShippingMethodEntity $shippingMethod): string => $shippingMethod->getTechnicalName());

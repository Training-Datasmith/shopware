...
class RefreshIndexCommand extends Command implements EventSubscriberInterface
{
    ...
    protected function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        $skipEntities = array_map(fn (string $indexer): string => str_replace('.indexer', '', $indexer), $skip);

        $onlyEntities = array_map(fn (string $indexer): string => str_replace('.indexer', '', $indexer), $only);

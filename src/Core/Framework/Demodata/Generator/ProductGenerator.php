...
class ProductGenerator implements DemodataGeneratorInterface
{
    ...
    private function buildVariants(array $combinations, array $prices, TaxCollection $taxes): array
    {
        ...
        foreach ($combinations as $options) {
            ...
            $variants[] = [
                ...
                'options' => array_map(fn ($id): array => ['id' => $id], $options),
            ];
        ...
        return [
            'children' => $variants,
            'configuratorSettings' => array_map(fn (string $id): array => ['optionId' => $id], array_filter(array_unique($configurator))),
        ];
    }
    ...
    private function buildVisibilities(): array
    {
        ...
        return array_map(fn (array $id): array => ['salesChannelId' => $id['id'], 'visibility' => ProductVisibilityDefinition::VISIBILITY_ALL], $ids);
    }
    ...
    private function buildProperties(array $properties): array
    {
        ...
        return array_map(fn ($config): array => ['id' => (string) $config], $productProperties);
    }

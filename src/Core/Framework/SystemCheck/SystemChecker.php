...
class SystemChecker
{
    ...
    {
        return array_map(function (BaseCheck $check): \Shopware\Core\Framework\SystemCheck\Check\Result {
            try {
                return $check->run();
            } catch (\Throwable $e) {
    ...
    {
        return array_map(
            fn (BaseCheck $check): \Shopware\Core\Framework\SystemCheck\Check\Result => new Result($check->name(), Status::SKIPPED, $message),
            $checks
        );
    }

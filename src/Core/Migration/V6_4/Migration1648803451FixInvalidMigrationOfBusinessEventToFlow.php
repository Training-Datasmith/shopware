...
class Migration1648803451FixInvalidMigrationOfBusinessEventToFlow extends MigrationStep
{
    ...
    public function update(Connection $connection): void
    {
        ...
        foreach ($invalidSequenceGroup as $sequence) {
            $actionSequence = array_values(array_filter($sequence, fn (array $sequence): bool => $sequence['action_name'] !== null))[0] ?? null;

            $parentCondition = array_values(array_filter($sequence, fn (array $sequence): bool => $sequence['rule_id'] !== null && $sequence['parent_id'] === null))[0] ?? null;
            ...
            $childrenCondition = array_values(array_filter($sequence, fn (array $sequence): bool => $sequence['rule_id'] !== null && $sequence['parent_id'] !== null));

...
class CartScopeDiscountPackager extends DiscountPackager
{
    ...
    {
        $allItems = $cart->getLineItems()->filter(fn (LineItem $lineItem): bool => $lineItem->getType() === LineItem::PRODUCT_LINE_ITEM_TYPE && $lineItem->isStackable());

        $priceDefinition = $discount->getPriceDefinition();
        if ($priceDefinition instanceof FilterableInterface && $priceDefinition->getFilter()) {
            $allItems = $allItems->filter(fn (LineItem $lineItem): bool => $priceDefinition->getFilter()->match(new LineItemScope($lineItem, $context)));
        }

    ...
    public function process(CartDataCollection $data, Cart $original, Cart $toCalculate, SalesChannelContext $context, CartBehavior $behavior): void
    {
        Profiler::trace('cart::promotion::process', function () use ($data, $original, $toCalculate, $context, $behavior): void {
            ...
            if ($toCalculate->getPrice()->getTotalPrice() === 0.0) {
                ...
                $discountPromotionsWithCode = $discountLineItems->filter(fn (LineItem $lineItem): bool => !$lineItem->hasPayloadValue('promotionCodeType') || $lineItem->getPayloadValue('promotionCodeType') !== PromotionItemBuilder::PROMOTION_TYPE_GLOBAL);
                if ($discountPromotionsWithCode->count() === 0) {
                    return;
                }

                $toCalculate->addErrors(
                    new PromotionsOnCartPriceZeroError($discountPromotionsWithCode->fmap(fn (LineItem $lineItem): ?string => $lineItem->getLabel()))
                );

...
class ContainerCartProcessor implements CartProcessorInterface
{
    ...
    private function calculate(LineItem $item, SalesChannelContext $context, LineItemCollection $scope): void
    {
        if ($item->getChildren()->count() > 0) {
            ...
            $this->calculateCollection($item->getChildren(), $context, fn (LineItem $item): bool => $item->getChildren()->count() > 0);

            $this->calculateCollection($item->getChildren(), $context, fn (LineItem $item): bool => $item->getPriceDefinition() instanceof QuantityPriceDefinition);

            $this->calculateCollection($item->getChildren(), $context, fn (LineItem $item): bool => $item->getPriceDefinition() instanceof CurrencyPriceDefinition);

            $this->calculateCollection($item->getChildren(), $context, fn (LineItem $item): bool => $item->getPriceDefinition() instanceof PercentagePriceDefinition);

...
class PaymentMethodCollection extends EntityCollection
{
    ...
    public function filterByActiveRules(SalesChannelContext $salesChannelContext): PaymentMethodCollection
    {
        ...
        return $this->filter(
            function (PaymentMethodEntity $paymentMethod) use ($salesChannelContext): bool {
    ...
    {
        return $this->fmap(fn (PaymentMethodEntity $paymentMethod): ?string => $paymentMethod->getPluginId());
    }

    public function filterByPluginId(string $id): self
    {
        return $this->filter(fn (PaymentMethodEntity $paymentMethod): bool => $paymentMethod->getPluginId() === $id);
    }

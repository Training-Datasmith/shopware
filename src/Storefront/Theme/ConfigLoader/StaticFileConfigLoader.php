...
class StaticFileConfigLoader extends AbstractConfigLoader
{
    ...
    {
        $fileObject['styleFiles'] = array_map(fn (array $file): object|string => (new File(''))->assign($file), $fileObject['styleFiles']);

        $fileObject['scriptFiles'] = array_map(fn (array $file): object|string => (new File(''))->assign($file), $fileObject['scriptFiles']);

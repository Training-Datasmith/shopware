...
class ThemeCompiler implements ThemeCompilerInterface
{
    ...
    private function getFeatureConfigScssMap(): string
    {
        ...
        $featuresScss = implode(',', array_map(fn ($value, string $key): string => \sprintf('"%s": %s', $key, json_encode($value, \JSON_THROW_ON_ERROR)), $allFeatures, array_keys($allFeatures)));
    ...
    {
        return array_map(fn ($value, string $key): string => \sprintf(
            '$%s: %s;',
            $key,
...
PHP_EOL;
    ...
    private function concatenateStyles(
        ...
        $styles = $styleFiles->map(fn (File $file): string => \sprintf('@import \'%s\';', $file->getFilepath()));
